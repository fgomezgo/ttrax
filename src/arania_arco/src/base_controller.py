...
def cmd_velCallback(twist_aux):
	pupr = rospy.Publisher('rover_left/setpoint/data', Float64, queue_size=10)
	pupl = rospy.Publisher('rover_left/setpoint/data', Float64, queue_size=10)
	twist = twist_aux
	...
	if(vel_x == 0):
		#drift
		right_vel = vel_th * whith_robot / 2.0
		left_vel = (-1) * rigth_vel
	elif(vel_th == 0):
		#frente/reversa
